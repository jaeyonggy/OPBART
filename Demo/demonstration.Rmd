...
```{r, results='hide'}
set.seed(7)
fitted_opsmbart = opsmbart(Y ~ . - X4 - X5, ~ X4 + X5, one_unit = 0.01, train_df, test_df, opts = opts)
```

```{r}
cat(sprintf(" us2: %.4f\n", mean(fitted_opsmbart$us_p[1:opts$num_save,2])), 
    sprintf("us3: %.4f\n", mean(fitted_opsmbart$us_p[1:opts$num_save,3])), 
    sprintf("us4: %.4f\n", mean(fitted_opsmbart$us_p[1:opts$num_save,4])))
```

```{r}
hpd_opsmbart = HPDinterval(mcmc(fitted_opsmbart$us_p[,2:4]))
rownames(hpd_opsmbart) = c("us2", "us3", "us4")
hpd_opsmbart
```

```{r}
# The regression coefficients for X4 and X5
cat(sprintf(" theta1: %.4f\n", mean(fitted_opsmbart$theta_p[1:opts$num_save,1])), 
    sprintf("theta2: %.4f\n", mean(fitted_opsmbart$theta_p[1:opts$num_save,2])))
```

```{r}
hpd_theta_opsmbart = HPDinterval(mcmc(fitted_opsmbart$theta_p))
rownames(hpd_theta_opsmbart) = c("theta1", "theta2")
hpd_theta_opsmbart
```

```{r}
confusionMatrix(fitted_opsmbart$test_preds, test_df$Y)
```

```{r}
# Marginal effects of X4
ME_X4 = matrix(round(colMeans(fitted_opsmbart$ME_train_list[[1]]), 4), ncol = 5)
colnames(ME_X4) = c("ME1", "ME2", "ME3", "ME4", "ME5")
ME_X4
```

```{r}
hpd_ME_X4 = round(HPDinterval(mcmc(fitted_opsmbart$ME_train_list[[1]])), 4)
...
ME_X5 = matrix(round(colMeans(fitted_opsmbart$ME_train_list[[2]]), 4), ncol = 5)
colnames(ME_X5) = c("ME1", "ME2", "ME3", "ME4", "ME5")
ME_X5
```

```{r}
hpd_ME_X5 = round(HPDinterval(mcmc(fitted_opsmbart$ME_train_list[[2]])), 4)
